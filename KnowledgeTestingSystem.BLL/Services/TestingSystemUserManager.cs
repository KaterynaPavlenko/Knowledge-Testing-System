...
namespace KnowledgeTestingSystem.BLL.Services
{
    ...
    public class TestingSystemUserManager : UserManager<UserEntity>
    {
        public TestingSystemUserManager(IUserStore<UserEntity> store)
            : base(store)
        {
        }

        public static TestingSystemUserManager Create(IdentityFactoryOptions<TestingSystemUserManager> options,
            IOwinContext context)
        {
            var manager =
                new TestingSystemUserManager(new UserStore<UserEntity>(context.Get<TestingSystemDbContext>()));
